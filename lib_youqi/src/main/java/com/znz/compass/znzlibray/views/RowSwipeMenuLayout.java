 ...
 * Date: 16/04/24
 */
public class RowSwipeMenuLayout extends ViewGroup {
    ...
    //存储的是当前正在展开的View
    private static RowSwipeMenuLayout mViewCache;
    ...
    public RowSwipeMenuLayout(Context context) {
        this(context, null);
    }

    public RowSwipeMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RowSwipeMenuLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public RowSwipeMenuLayout setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public RowSwipeMenuLayout setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static RowSwipeMenuLayout getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.RowSwipeMenuLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.RowSwipeMenuLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.RowSwipeMenuLayout_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.RowSwipeMenuLayout_leftSwipe) {
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = RowSwipeMenuLayout.this;
