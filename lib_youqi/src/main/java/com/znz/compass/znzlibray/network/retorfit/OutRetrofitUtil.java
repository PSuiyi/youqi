...
public class OutRetrofitUtil {
    ...
    public static OutRetrofitUtil getInstance(boolean withCommonParams) {
        if (INSTANCE == null || withCommonParams != WITH_COMMON_PARAMS) {
            INSTANCE = new OutRetrofitUtil(withCommonParams);
        }
        return INSTANCE;
    }

    private OutRetrofitUtil(boolean withCommonParams) {
...
//            builder.cache(cache).addInterceptor(cacheInterceptor);
            ...
            Interceptor addQueryParameterInterceptor = chain -> {
                ...
                HttpUrl modifiedUrl = originalRequest.url().newBuilder()
                        .addQueryParameter("password", "mobile123")
                        .build();
