...
public class BaseModel<V extends IView> implements IModel {
    ...
    public void request(Observable<ResponseBody> observable, ZnzHttpListener listener, @LODING_TYPE int lodingType) {
        request(observable, listener, lodingType, 1);
    }

    public void requestZnz(Observable<ResponseBody> observable, ZnzHttpListener listener) {
        requestZnz(observable, listener, LODING_NONE);
    }

    public void requestZnz(Observable<ResponseBody> observable, ZnzHttpListener listener, @LODING_TYPE int lodingType) {
        request(observable, listener, lodingType, 2);
    }
     ...
     * @param lodingType
     */
    public void request(Observable<ResponseBody> observable, ZnzHttpListener listener, @LODING_TYPE int lodingType, int serviceType) {
...
//                mDataManager.showToast(e.getMessage());
            ...
            public void onNext(ResponseBody responseBody) {
                handleResult(responseBody, listener, lodingType, serviceType);
            }
        };
     ...
     * @param serviceType
     */
    private void handleResult(ResponseBody responseBody, ZnzHttpListener listener, @LODING_TYPE int lodingType, int serviceType) {
        try {
            ...
            switch (serviceType) {
                case 1:
                    ...
                    } else if (responseJson.getString("status_code").equals("2222")) {
                        mDataManager.showInfoRemind(context);
                    } else {
